...
/*
 * Plugin Name: Replace wp-login.php
 * Plugin URI:  https://geekinc.ca
 * Description: Change wp-login.php to whatever you want.
 * Version:     2.5.5
 * Author:      Ben Inkster
 * Author URI:  https://geekinc.ca
 * Text Domain: replace-wp-login
 * Domain Path:
 * Network:
 * License:     GPL-2.0+
 */

if ( defined( 'ABSPATH' ) && ! class_exists( 'Replace_WP_Login' ) ) {

	class Replace_WP_Login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'Replace_WP_Login', 'uninstall' ) );
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'replace-wp-login' ), '<strong>' . __( 'Rename wp-login.php', 'replace-wp-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'Rename wp-login.php', 'replace-wp-login' ) . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'replace-wp-login' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row">' . __( 'Networkwide default', 'replace-wp-login' ) . '</th>';
		...
		public function admin_init() {
			...
			add_settings_section(
				'replace-wp-login-section',
				__( 'Rename wp-login.php', 'replace-wp-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'replace-wp-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			...
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to %s.', 'replace-wp-login' ), '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Network Settings', 'replace-wp-login' ) . '</a>') . '</p>';
			}
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'replace-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}

		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Settings', 'replace-wp-login' ) . '</a>' );
			} elseif ( ! is_network_admin() ) {
				array_unshift( $links, '<a href="' . admin_url( 'options-permalink.php#rwl-page-input' ) . '">' . __( 'Settings', 'replace-wp-login' ) . '</a>' );
			}
		...
		public function plugins_loaded() {
			...
			load_plugin_textdomain( 'replace-wp-login' );
			...
			if (
				...
				wp_die( __( 'This feature is not enabled.', 'replace-wp-login' ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'replace-wp-login' ) );
			}
			...
			if (
				...
				$FILE = 'wp-login.php';
                @require_once ABSPATH . 'wp-content/plugins/replace-wp-login/includes/' . $FILE;
